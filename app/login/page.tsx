...
function LoginUser() {
  ...
  const  messageEmail = !emailValid ? "Tu email debe tener u mínimo de 12 caracteres." : "";
    const validEmail = emailValid;
   const messagePassword= !passwordValid ? "Tu contraseña debe tener 8 o más caracteres." : "";
   const validPassword =passwordValid;

    setInputsErrors({email:{valid: validEmail, message: messageEmail}, password:{valid: validPassword, message: messagePassword}});
      ...
      <div>
        ...
        <form action="" className="flex flex-col text-xs" onSubmit={handleSubmit} >
            <div className="flex flex-col relative">
              ...
              />
        <p className={`text-delete-hover absolute top-full ${!inputsErrors.email.valid ? 'visible':'invisible'}`}>
          {inputsErrors.email.message}
            ...
            />
        <p className={`text-delete-hover absolute top-full ${!inputsErrors.password.valid ? 'visible':'invisible'}`}>
